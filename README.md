...
・MVPでログイン設定は実装するが、今後しなくても本リリースのSNS投稿、推し議員登録以外は利用できるようにセキュリティ対策をする。
...
　・ニュース連携：RSSで取得
・デプロイはRender
・ログイン設定はSNS投稿、推し議員登録で実装　→　Devise + OmniAuth（セキュリティ対策をして、最初はログインしなくても使えるようにリリースをする。今後状況次第でログイン機能追加も視野に入れる）
